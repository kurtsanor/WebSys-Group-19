import {
  getShowsByGenreAndPage,
  searchShowsByNameAndPage,
...
document.querySelectorAll(".movies__filter-item").forEach((item) => {
  item.addEventListener("click", async () => {
    ...
    const shows =
      genreId != "0"
        ? await getShowsByGenreAndPage(genreId, 1)
        : await getShowsByGenreAndPage(null, 1);
...
async function getTrailerUrl(movieId) {
  const data = await getTrailerByIdAndType(movieId, "tv");
